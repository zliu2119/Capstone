...
bb = addmf(bb, 'input', 1, 'near', 'gaussmf', [40 1]);
...
bb = addmf(bb, 'input', 2, 'high', 'gaussmf', [50 120]);
...
for ii=1:3
  fprintf('%f\t%f\t%d\t\t%f: %s\n' , x(ii,:) , res1(ii) , decided(round(ii),:)) ;
endfor

x = [] ;
prompt = {"Distance (m): ", "Speed (km/h): " , "Light: (1=g, 2=y, 3=r)"};
rowscols = [1,10; 1,10 ; 1,20];

dims = inputdlg (prompt, "Enter Data", rowscols);
if (prod(size(dims)) ~= 0) 
  x = [ str2num( cell2mat(dims(1))) , str2num( cell2mat(dims(2))) , str2num( cell2mat(dims(3)))];
endif

while (prod(size(dims)) ~= 0)
    zans = evalfis(x,bb) ;
    fprintf('\nAnswer = %f ==> %s\n',zans , decided(round(zans),:) ) ;
    msgbox (['Answer = ' , num2str(zans) , ' ==> ' , decided(round(zans),:)]) ;
    dims = inputdlg (prompt, "Enter Data", rowscols);
    if (prod(size(dims)) ~= 0) 
      x = [ str2num( cell2mat(dims(1))) , str2num( cell2mat(dims(2))) , str2num( cell2mat(dims(3)))];
    endif
