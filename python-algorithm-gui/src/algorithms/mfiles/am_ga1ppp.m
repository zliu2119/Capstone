...
prompt = {"How many Queens?", "Population size?" , "No of iterations?" , "Mutation rate? (0 to 1)" , ...
 "Printing the Fitness? (y/n)" , "Dynamic Mutation Rate? (y/n)" , "Is Mutation above 1 allowed? (y/n)" , ...
 "Population STD Factor= " , "Mutation Increment Factor= "};
rowscols = [1,10; 1,10 ; 1,10 ; 1,10 ; 1,10 ;  1,10 ;  1,10 ; 1,10 ; 1,10 ] ;
...
dmr = cell2mat(dims(6)) ;
mutbg1 = cell2mat(dims(7)) ;
pstdfct = str2num( cell2mat(dims(8))) ;
muifct = str2num( cell2mat(dims(9))) ;


pup = randi(N,M,N) ;
fn_array = compute_fitness(pup,prsw) ;
org_std = std(fn_array) ;
lev2 = min(50,M) ;
